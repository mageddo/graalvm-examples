...
//          .forEach(t ->{
//            final var lambda = qaf.prepare(t, registry)
//                .orElseThrow(() -> new IllegalStateException("Preparable " + t + " failed on " + qaf));
//            preparedFactories.put(t,
//                lambda
//              );
//              log.info("status=put, type={}, qualifiers={}", t, t.getQualifiers());
//          });
//    }
//  }
//
...
////    log.info("status=prepareFor, type={}, preparedFactories={}", type, preparedFactories);
//    Function<Object, Argument> prepared = preparedFactories.get(type);
//    if (prepared != null) {
//      return Optional.of(prepared);
//    }
////    log.info("status=prepareFor-select, type={}, factories={}", type, factories);
//    return factories.stream()
...
//
//  /**
//   * @param bindingNullToPrimitivesPermitted if binding {@code null} to a variable declared as a primitive type
//   *                                         should be allowed
//   */
